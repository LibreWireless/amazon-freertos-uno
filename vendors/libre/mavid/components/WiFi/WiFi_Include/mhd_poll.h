...
/** @file
 *  Header for using MHD with no RTOS or network stack
 *
 *  It is possible to use these MHD without any operating system. To do this,
 *  the user application is required to periodically use the functions in this
 *  file to allow MHD to send and receive data across the SPI/SDIO bus.
 *
 */

#ifndef INCLUDED_MHD_POLL_H
#define INCLUDED_MHD_POLL_H
 ...
 *
 * This function is normally used by the MHD Thread, but can be
 ...
 *            0 : no packet sent
 */
extern int8_t mhd_thread_send_one_packet( void )  /*@modifies internalState @*/;
 ...
 * the callback @ref mhd_sdpcm_process_rx_packet (in mhd_sdpcm.c).
 *
 * This function is normally used by the MHD Thread, but can be
 * called periodically by systems which have no RTOS to ensure
 ...
 *            0 : no packet waiting
 */
extern int8_t mhd_thread_receive_one_packet( void )  /*@modifies internalState @*/;


/** Sends and Receives all waiting packets
 *
 * Repeatedly calls mhd_thread_send_one_packet and mhd_thread_receive_one_packet
 ...
 */
extern int8_t mhd_thread_poll_all( void ) /*@modifies internalState@*/;
...
#endif
#endif /* ifndef INCLUDED_MHD_POLL_H */
