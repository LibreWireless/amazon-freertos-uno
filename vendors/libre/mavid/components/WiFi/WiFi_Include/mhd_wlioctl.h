...
#define WLC_PHY_SAMPLE_COLLECT             ( (uint32_t) 307 )
#define WLC_SET_INTERFERENCE_OVERRIDE_MODE	( (uint32_t) 311 )
#define WLC_GET_INTERFERENCE_OVERRIDE_MODE	( (uint32_t) 312 )
#define WLC_GET_WSEC_PMK                   ( (uint32_t) 318 )
#define WLC_GET_RANDOM_BYTES               ( (uint32_t) 319 )
#define WLC_LAST                           ( (uint32_t) 342)
...
#if defined(MHD_WL_SAMPLE_COLLECT)
...
typedef struct wl_samplecollect_args {
	...
	uint16_t version;     /* see definition of WL_SAMPLECOLLECT_T_VERSION */
...
/* version for unpacked sample data, int16_t {(I,Q),Core(0..N)} */
#define	WL_SAMPLEDATA_T_VERSION_SPEC_AN 2

typedef struct wl_sampledata {
	uint16_t version;	/* structure version */
	uint16_t size;	/* size of structure */
	uint16_t tag;	/* Header/Data */
	uint16_t length;	/* data length */
	uint32_t flag;	/* bit def */
} wl_sampledata_t;
#endif
