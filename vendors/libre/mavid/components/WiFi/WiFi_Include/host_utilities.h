...
#define HOST_VERIFY(x)                               {host_result_t res = (x); if (res != HOST_SUCCESS){return res;}}
...
#ifdef HOST_ENABLE_MALLOC_DEBUG
#include <stddef.h>
#include "mhd_rtos.h"
...
#endif /* ifdef HOST_ENABLE_MALLOC_DEBUG */

/* Define macros to assist operation on host MCUs that require aligned memory access */
#ifndef HOST_REQUIRES_ALIGNED_MEMORY_ACCESS

#define HOST_MEMCPY(destination, source, size)   memcpy(destination, source, size)

#define HOST_WRITE_16( pointer, value )      (*((uint16_t*)pointer) = value)
#define HOST_WRITE_32( pointer, value )      (*((uint32_t*)pointer) = value)
#define HOST_READ_16( pointer )              *((uint16_t*)pointer)
#define HOST_READ_32( pointer )              *((uint32_t*)pointer)

#else /* HOST_REQUIRES_ALIGNED_MEMORY_ACCESS */

#define HOST_MEMCPY( destination, source, size )   mem_byte_cpy( destination, source, size )

#define HOST_WRITE_16( pointer, value )      do { ((uint8_t*)pointer)[0] = (uint8_t)value; ((uint8_t*)pointer)[1]=(uint8_t)(value>>8); } while(0)
#define HOST_WRITE_32( pointer, value )      do { ((uint8_t*)pointer)[0] = (uint8_t)value; ((uint8_t*)pointer)[1]=(uint8_t)(value>>8); ((uint8_t*)pointer)[2]=(uint8_t)(value>>16); ((uint8_t*)pointer)[3]=(uint8_t)(value>>24); } while(0)
#define HOST_READ_16( pointer )              (((uint8_t*)pointer)[0] + (((uint8_t*)pointer)[1] << 8))
#define HOST_READ_32( pointer )              (((uint8_t*)pointer)[0] + ((((uint8_t*)pointer)[1] << 8)) + (((uint8_t*)pointer)[2] << 16) + (((uint8_t*)pointer)[3] << 24))

#endif /* HOST_REQUIRES_ALIGNED_MEMORY_ACCESS */

/******************************************************
 *                    Constants
 ******************************************************/

#define HOST_NEVER_TIMEOUT   (0xFFFFFFFF)
#define HOST_WAIT_FOREVER    (0xFFFFFFFF)
#define HOST_NO_WAIT         (0)
