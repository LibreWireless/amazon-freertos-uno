...
/** @file
 *  Defines the SDIO part of the HOST Platform Interface.
 *
 *  Provides constants and prototypes for functions that
 *  enable Host to use an SDIO bus on a particular hardware platform.
 */
...
#ifndef SDIO_F2_BLOCK_SIZE
// Org, stable
//#define SDIO_F2_BLOCK_SIZE    SDIO_128B_BLOCK

//HT New , stable
#define SDIO_F2_BLOCK_SIZE      SDIO_512B_BLOCK
#endif
 ...
 *  Functions that enable HOST to use an SDIO bus
 ...
 * platform in use.
 * HOST uses this function as a generic way to transfer data
 * across an SDIO bus.
 ...
 *
 * @return MHD_SUCCESS if successful, otherwise an error code
 */
extern mhd_result_t host_platform_sdio_transfer( mhd_bus_transfer_direction_t direction, sdio_command_t command, sdio_transfer_mode_t mode, sdio_block_size_t block_size, uint32_t argument, /*@null@*/ uint32_t* data, uint16_t data_size, sdio_response_needed_t response_expected, /*@out@*/ /*@null@*/ uint32_t* response );
 ...
 *
 */
extern mhd_result_t host_platform_sdio_enumerate( void );
 ...
 *
 * This function is implemented by Host and must be called
 ...
 *
 * This function is called by HOST to unmask the bus interrupt
 * on host platforms that must mask off the bus interrupt to
 * allow processing of the existing interrupt.
 */
extern mhd_result_t host_platform_unmask_sdio_interrupt( void );


#ifndef  HOST_DISABLE_MCU_POWERSAVE
 ...
 *
 */
extern mhd_result_t host_enable_oob_interrupt( void );
extern mhd_result_t host_disable_oob_interrupt( void );
...
#endif /* ifndef  HOST_DISABLE_MCU_POWERSAVE */
/** @} */
/** @} */
